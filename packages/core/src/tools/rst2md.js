...
var block = {
  ...
  // Cosumes text which is considered a comment:
  //
  //		.. comment
  //
  comment: /^\.{2}\s+([^\n]+|\n\s+[^\n]+)(?:\n+|$)/,
...
var listhr = /\n+(?=(?: *[=\-`:\.~'"\^_*+#]) {3,} *(?:\n+|$))/;
var listbullet = /^ *([*+\-•‣⁃]|\(?[A-Za-z0-9]+[\.\)]) +/; // Does not match Roman numerals as list bullets

block.item = replace(block.item, 'gm')(/bull/g, block.bullet)();
block.list = replace(block.list)(/bull/g, block.bullet)('hr', listhr)();
...
block.directives = {
  ...
  parts: ['contents', 'sectnum', 'section-autonumbering', 'header', 'footer'],
  references: ['taget-nodes', 'footnotes', 'citations'],
  html: ['meta'],
  substitution: ['replace', 'unicode', 'date'],
  misc: ['include'],
};
...
block.token = function (rst, tokens) {
  ...
  var m, l, i, d, item, fields, options, space, loose, term, next, directive, args, text;
...
function markdown(tokens) {
  var doc = {
    ...
    space: function (up) {
      return this.blockDepth
        ? repeatString(' > ', this.blockDepth)
        : '' +
            (this.indent ? repeatString('  ', this.indent) : '') +
            (this.lists.length
              ? this.lists[this.lists.length - 1].ordered
  ...
  var blockLexers = {
    ...
    section: function (doc, token) {
      doc.add(
        doc.heading.substring(0, token.depth) + ' ' + token.text + ' ' + doc.heading.substring(0, token.depth) + '\n',
      );
      doc.add('\n');
    ...
    comment: function (doc, token) {
      doc.add('<!-- ' + token.comment + ' -->');
    },
    ...
    definitionStart: function (doc, token) {
      if (options.definitionListAsHTML) {
        doc.add('<dl>\n');
        doc.add(
          '  <dt>' + token.term + (token.classifiers.length ? ' : ' + token.classifiers.join(' : ') : '') + '</dt>\n',
        );
      } else {
